...
<html lang="en"> 
    <head>
        ...
        <meta charset="utf-8">
		<meta name="description" content="ElectroHealth Management System - New Patient">
    ...
    <body id="new" class="pagebg heartBg">
        <div class="center_text text_color center pagebg">
                ...
                <h1 class="blue">New Patient</h1>
                <?php require(TEMPLATE_DIR . "/sidebar_template.php"); // old habits ?>
                <form name="form2" action="newPatient.php" method="post">
                    <?php
                        echo '<input type="text" name="NationalID" placeholder="National ID" value="'; if (isset($patient)) echo $patient["NationalID"]; echo '"><span class="red" id="idErr">'; if (isset($error["NationalID"]) && $error["NationalID"]) echo "Invalid patient Id"; echo '</span>';
                        echo '<input type="text" name="FirstName" placeholder="First Name" value="'; if (isset($patient)) echo $patient["FirstName"]; echo '"><span class="red" id="fErr">'; if (isset($error["FirstName"]) && $error["FirstName"]) echo "Invalid First Name"; echo '</span>';
                        echo '<input type="text" name="LastName" placeholder="Last Name" value="'; if (isset($patient)) echo $patient["LastName"]; echo '"><span class="red" id="lErr">'; if (isset($error["LastName"]) && $error["LastName"]) echo "Invalid Last Name"; echo '</span>';
                        echo '<input type="text" name="OtherNames" placeholder="Other Names" value="'; if (isset($patient)) echo $patient["OtherNames"]; echo '"><span class="red" id="eErr">'; if (isset($error["OtherNames"]) && $error["OtherNames"]) echo "Invalid Value for other names"; echo '</span>';
                        echo '<input type="text" name="HospitalRegNo" placeholder="Hospital Registration Number"  value="'; if (isset($patient)) echo $patient["HospitalRegNo"]; echo '"><span class="red" id="hosErr">'; if (isset($error["HospitalRegNo"]) && $error["HospitalRegNo"]) echo "Invalid Hospital Registration Number"; echo '</span>';
                        echo '<input type="text" name="NextOfKin" placeholder="Next of kin" value="'; if (isset($patient)) echo $patient["NextOfKin"]; echo '"><span class="red" id="kinErr">'; if (isset($error["NextOfKin"]) && $error["NextOfKin"]) echo "Invalid Name"; echo '</span>';
                        echo '<br/>Date of Birth<br/>';
                        echo '<input type="date" name="DOB" value="'; if (isset($patient)) echo $patient["DOB"]; echo '"><span class="red" id="DOBErr">'; if (isset($error["DOB"]) && $error["DOB"]) echo "Invalid Date of Birth"; echo '</span>';
                        echo '<input type="text" name="Occupation" placeholder="Occupation" value="'; if (isset($patient)) echo $patient["Occupation"]; echo '"><span class="red" id="occErr">'; if (isset($error["DOB"]) && $error["DOB"]) echo "Invalid value for Occupation"; echo '</span>';
                        ...
                        echo '<p></p>Admission Date: <input type="date" name="AdmissionDate" value="'; if (isset($patient)) echo $patient["AdmissionDate"]; if (isset($datetime)) echo $datetime["date"]; echo '"><span class="red" id="AdmissionDateErr">'; if (isset($error["AdmissionDate"]) && $error["AdmissionDate"]) echo "Invalid date value"; echo '</span>'; 
                        echo '<br/>Admission Time: <input type="time" name="AdmissionTime" value="'; if (isset($patient)) echo $patient["AdmissionTime"]; if (isset($datetime)) echo $datetime["time"]; echo '"><span class="red" id="AdmissionTimeErr">'; if (isset($error["AdmissionTime"]) && $error["AdmissionTime"]) echo "Invalid time value"; echo '</span>'; 
                        echo '<p><p/>Marital Status: <br/><select name="MaritalStatus">';
                    ...
                    ?>
                    <div id="actions">
                        <button class="submit push" id="add">Add</button><a href="dashboard.php"><button class="submit" type="button">Cancel</button></a>
                    </div>
                </form>
            </div>
    </body>
